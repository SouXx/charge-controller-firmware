...
void control_off_to_active_if_everything_fine()
{
    PowerPort port = {};
    ...
    TEST_ASSERT_EQUAL(true, load.active);
    TEST_ASSERT_EQUAL(LOAD_STATE_ON, load.state);       // deprecated
}

void control_active_to_off_overvoltage()
{
    PowerPort port = {};
    LoadOutput load(&port);
    load_init(&load);
    ...
    TEST_ASSERT_EQUAL(false, load.active);
...
void control_active_to_off_overcurrent()
{
    PowerPort port = {};
    LoadOutput load(&port);
    load_init(&load);
...
void control_active_to_off_voltage_dip()
{
    PowerPort port = {};
    LoadOutput load(&port);
    load_init(&load);
...
void control_active_to_off_int_temp()
{
    PowerPort port = {};
    LoadOutput load(&port);
    load_init(&load);
...
void control_active_to_off_bat_temp()
{
    PowerPort port = {};
    LoadOutput load(&port);
    load_init(&load);
    ...
    load.control();
    TEST_ASSERT_EQUAL(false, load.active);
    TEST_ASSERT_EQUAL(false, load.usb_active);
    TEST_ASSERT_EQUAL(LOAD_STATE_OFF_TEMPERATURE, load.state);      // deprecated
...
void control_active_to_off_low_soc()
{
    PowerPort port;
    LoadOutput load(&port);
    load.state = LOAD_STATE_ON;
...
void control_active_to_off_if_enable_false()
{
    PowerPort port = {};
    LoadOutput load(&port);
    load_init(&load);
    ...
    load.control();
    TEST_ASSERT_EQUAL(true, load.active);
    TEST_ASSERT_EQUAL(true, load.usb_active);
...
void control_off_overvoltage_to_on_at_lower_voltage()
{
    ...
    port.voltage = port.sink_voltage_max + 0.4;     // test hysteresis
    load.control();
    TEST_ASSERT_EQUAL(ERR_LOAD_OVERVOLTAGE, dev_stat.error_flags);
    TEST_ASSERT_EQUAL(LOAD_STATE_OFF_OVERVOLTAGE, load.state);
...
void load_tests()
{
    ...
    // control tests
    RUN_TEST(control_off_to_active_if_everything_fine);
    RUN_TEST(control_active_to_off_overvoltage);
    RUN_TEST(control_active_to_off_overcurrent);
    RUN_TEST(control_active_to_off_voltage_dip);
    RUN_TEST(control_active_to_off_int_temp);
    RUN_TEST(control_active_to_off_bat_temp);
    RUN_TEST(control_active_to_off_low_soc);
    RUN_TEST(control_active_to_off_if_enable_false);
